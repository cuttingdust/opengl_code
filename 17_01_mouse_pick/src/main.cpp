...
void mouseClick_callback(GLFWwindow* windows, int button, int action, int mods){
    if ((action == GLFW_PRESS) && (button == GLFW_MOUSE_BUTTON_RIGHT)) {
        double winX, winY;
        float winZ;
        glfwGetCursorPos(windows, &winX, &winY);
        glReadPixels(
                     (int)winX,
        ...
        float x = (2.0f * winX) / SCR_HEIGHT - 1.0f;
        ...
        if (winZ < 1.0f) {
            ...
            glm::vec4 worldPosition(x, y, z, 1);
            worldPosition *= w;
            worldPosition = glm::inverse(view) * glm::inverse(projection) * worldPosition;
            
            std::cout << " x:" << worldPosition.r
                << " y:" << worldPosition.y
                << " z:" << worldPosition.z
                << " w:" << w << std::endl;
            
        } else {
            std::cout << "没有像素被选中" << std::endl;
        }
    }
}
