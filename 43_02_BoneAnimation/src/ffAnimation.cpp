...
namespace FF
{
	ffAnimation::ffAnimation(const aiAnimation* _aiAnim , aiNode* _root , std::map<std::string, ffBoneInfo>& _boneInfoMap, uint& _boneCount)
	{
	...
	void ffAnimation::readAnimationData(const aiAnimation* _aiAnim, std::map<std::string, ffBoneInfo>& _boneInfoMap, uint& _boneCount)
	{
		for (uint i = 0; i < _aiAnim->mNumChannels; i++)
		{
			...
			{
				_boneInfoMap[_boneName].m_id = _boneCount;
				_boneCount++;
			}
